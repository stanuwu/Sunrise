...
constexpr SettingsRange<std::int32_t> kFieldOfView{55, 105};
...
[[nodiscard]] static bool within(Value value, SettingsRange<Value> range) noexcept {
...
[[nodiscard]] static bool contains(const std::array<Value, Count>& values,
 ...
 */
[[nodiscard]] static bool valid_controls(const Controls& value) noexcept {
 ...
 * @return True when every number belongs to its supported domain.
 */
[[nodiscard]] static bool valid_audio(const Audio& value) noexcept {
 ...
 * @return True when every number belongs to its supported domain.
 */
[[nodiscard]] static bool valid_display(const Display& value) noexcept {
 ...
 * @return True when every number belongs to its supported domain.
 */
[[nodiscard]] static bool valid_interface(const Interface& value) noexcept {
 ...
 * @return True when every number belongs to its supported domain.
 */
[[nodiscard]] static bool valid_social(const Social& value) noexcept {
...
[[nodiscard]] static bool valid_input_code(const std::optional<std::uint16_t>& value) noexcept {
    if (!value.has_value()) {
        return true;
    }
...
[[nodiscard]] static bool valid_key_bindings(const bindings::KeyBindings& value) noexcept {
    if (!value.configured) {
        return false;
    }
