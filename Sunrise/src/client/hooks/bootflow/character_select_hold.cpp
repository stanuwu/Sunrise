...
/**
 * Attaches the character-select hold: without it, the sign-in boot step clears its own stay
 * flag on entry and the screen falls straight through to orbit, whatever character was picked,
 * created, or removed. The client's exit gate reads this flag every frame and only leaves the
 * screen once something (the player picking a character) sets it again.
